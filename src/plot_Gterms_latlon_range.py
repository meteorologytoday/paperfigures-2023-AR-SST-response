...
for s, sname in enumerate(["clim", "AR"]):
    ...
    for i, varname in enumerate(plotted_varnames):
        ...
        _ax.bar(x_pos + i*bar_width, _data[selected_idx, 0],   bar_width, label=plot_infos[varname]['label'], **kwargs)

        #print(ds)
        _anom_ARpARf_data = ds_stat["AR+ARf"][varname].to_numpy()[selected_idx, :] 
        _anom_AR_data = ds_stat["AR"][varname].to_numpy()[selected_idx, :]
        _anom_ARf_data = ds_stat["ARf"][varname].to_numpy()[selected_idx, :]
        _mean_data = ds_stat["clim"][varname].to_numpy()[selected_idx, :]

        # error bar
        _error_bar_lower = - _anom_AR_data[selected_idx, 1]
        _error_bar_upper =   _anom_AR_data[selected_idx, 1]
        _offset = _data[selected_idx, 0]
